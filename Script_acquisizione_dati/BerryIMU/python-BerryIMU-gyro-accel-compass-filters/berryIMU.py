...
#    http://ozzmaker.com/
"""
    Script per acquisire i dati dalla piattaforma inerziale berryIMU.
    Versione originale: https://github.com/ozzmaker/BerryIMU.git)
    I dati sono comunicati via I2C e scritti su un file di log ogni 40 millisecondi;
    i dati salvati sono:
    -) timestamp;
    -) angolo polare rispetto al nord magnetico e angolo di tilt (?)
    -) angolo di rotazione rispetto al lato lungo
    -) angolo di rotazioen rispetto al lato corto
    ...
    Lo script è pensato per scrivere in modo continuo dalla IMU: se dovesse
    esserci un problema di connessione, siccome è connessa tramite I2C, si aspetta
    finchè non torna la connessione e continua a scrivere sul file (CONTROLLARE).
"""
...
fname = os.path.join("home","pi","Desktop","IMU.log")
log_file = open(fname, "a")
log_file.write("#"+str(datetime.datetime.today())+"\n\r")
log_file.write("#Start_time[s]\tHeading[deg]\ttiltCompensatedHeading[deg]\tkalmanX[deg]\tkalmanY[deg]\n\r")
log_file.close()
...
while True:
    ...
    outputString = "{:.5f}".format(compute_timestamp(a))
